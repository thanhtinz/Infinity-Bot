...
function getWebhookClient(url) {
    if (!_webhookClients.has(url)) {
        _webhookClients.set(url, new WebhookClient({ url }));
    }
    return _webhookClients.get(url);
}

async function sendLog(webhookUrl, logName, botName, botAvatarURL, container) {
    ...
    try {
        ...
        console.error(`[BOT LOGGER] ${logName} webhook failed:`, err.message);
    }
}

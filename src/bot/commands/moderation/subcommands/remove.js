...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!targetMember.roles.cache.has(role.id))
      return modReply(interaction, await tg(guildId, 'moderation.remove.roleNotFoundTitle'), await tg(guildId, 'moderation.remove.roleNotFoundBody'), true);

    try {
      await targetMember.roles.remove(role, `[ROLEREMOVE] By ${interaction.user.tag}`);
      await modReply(interaction, await tg(guildId, 'moderation.remove.successTitle'),
        await tg(guildId, 'moderation.remove.success', { user: `${targetUser}`, role: role.name, moderator: interaction.user.tag }));
    } catch (error) {
      const msg = error.code === 50013 ? await tg(guildId, 'moderation.remove.errorPermission') : await tg(guildId, 'moderation.remove.errorGeneric');

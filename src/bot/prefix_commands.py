...
class PrefixCommandsCog(discord.Cog):
    ...
    async def _dispatch(self, ctx: PrefixContext, cmd: str, args: list[str]) -> bool:
        ...
        # Leveling: !rank [user], !level rank [user], !level leaderboard, !levels
        if cmd in {"rank", "levels"} or (cmd == "level" and args):
            if not await self._guard(ctx, "leveling"):
                return True
            cog = self._cog("LevelingCog")
            if not cog:
                return False
            if cmd == "rank":
                await cog.rank(ctx, user=_find_member(ctx, args[0] if args else None))
            elif cmd == "levels":
                await cog.leaderboard(ctx, page=_positive_int(args[0], 1) or 1)
            else:
                sub = args[0].lower()
                if sub == "rank":
                    await cog.rank(ctx, user=_find_member(ctx, args[1] if len(args) > 1 else None))
                elif sub in {"leaderboard", "top", "lb"}:
                    await cog.leaderboard(ctx, page=_positive_int(args[1], 1) if len(args) > 1 else 1)
                else:
                    await ctx.respond(f"Dùng: `{ctx.prefix}rank [@user]` hoặc `{ctx.prefix}level leaderboard`", delete_after=12)
            return True

        # Ticket public commands. Staff/admin ticket actions remain slash-only.
        if cmd == "ticket":
            if not await self._guard(ctx, "ticket"):
                return True
            cog = self._cog("TicketCog")
            if not cog:
                return False
            sub = args[0].lower() if args else "create"
            if sub == "create":
                await cog.ticket_create(ctx, subject=" ".join(args[1:]) if len(args) > 1 else None)
            elif sub == "history":
                await cog.ticket_history(ctx, user=_find_member(ctx, args[1] if len(args) > 1 else None))
            elif sub == "stats":
                await cog.ticket_stats(ctx)
            else:
                await ctx.respond(f"Dùng: `{ctx.prefix}ticket create [chủ đề]`, `{ctx.prefix}ticket history`, `{ctx.prefix}ticket stats`", delete_after=12)
            return True

        # Temp voice user room commands. They are member self-service, so prefix-enabled.
        if cmd == "room":
            if not await self._guard(ctx, "temp_voice"):
                return True
            cog = self._cog("TempVoiceCog")
            if not cog or not args:
                await ctx.respond(f"Dùng: `{ctx.prefix}room lock|unlock|hide|unhide|rename|limit|owner|claim|panel`", delete_after=12)
                return True
            sub = args[0].lower()
            method_map: dict[str, Callable[..., Awaitable]] = {
                "lock": cog.lock, "unlock": cog.unlock, "hide": cog.hide, "unhide": cog.unhide,
                "private": cog.private, "public": cog.public, "claim": cog.claim, "owner": cog.owner,
            }
            if sub in method_map:
                await method_map[sub](ctx)
            elif sub == "rename":
                await cog.rename(ctx, name=" ".join(args[1:]) or "Phòng của bạn")
            elif sub == "limit":
                await cog.limit(ctx, so=_positive_int(args[1] if len(args) > 1 else None, 0) or 0)
            elif sub == "bitrate":
                await cog.bitrate(ctx, kbps=_positive_int(args[1] if len(args) > 1 else None, 64) or 64)
            elif sub in {"permit", "reject", "boot", "mute", "unmute", "transfer"}:
                member = _find_member(ctx, args[1] if len(args) > 1 else None)
                if not member:
                    await ctx.respond(f"Dùng: `{ctx.prefix}room {sub} @user`", delete_after=10)
                else:
                    fn = {
                        "permit": cog.permit, "reject": cog.reject, "boot": cog.boot_voice,
                        "mute": cog.mute_voice, "unmute": cog.unmute_voice, "transfer": cog.transfer,
                    }[sub]
                    await fn(ctx, user=member)
            elif sub == "panel":
                await cog.panel(ctx, channel=_find_text_channel(ctx, args[1] if len(args) > 1 else None))
            else:
                await ctx.respond(f"Không rõ room command `{sub}`.", delete_after=10)
            return True

...
const LAYER_NAME_KEYS: Record<string, string> = {
  "Glass panel": "lv_layerGlassPanel",
  "Avatar": "lv_layerAvatar",
  "Display name": "lv_layerDisplayName",
  "Username": "lv_layerUsername",
  "Server": "lv_layerServer",
  "Rank capsule": "lv_layerRankCapsule",
  "Rank text": "lv_layerRankText",
  "Level capsule": "lv_layerLevelCapsule",
  "Level text": "lv_layerLevelText",
  "Progress bar": "lv_layerProgressBar",
  "XP text": "lv_layerXpText",
  "Percent": "lv_layerPercent",
  "Stat capsule": "lv_layerStatCapsule",
  "Text layer": "lv_layerText",
  "Shape": "lv_layerShape",
  "Progress": "lv_layerProgress",
};

const LAYER_DESC_KEYS: Record<string, string> = {
  "Large display name": "lv_layerDescDisplayName",
  "Small username": "lv_layerDescUsername",
  "Name server": "lv_layerDescServer",
  "Avatar user": "lv_layerDescAvatar",
  "XP progress bar": "lv_layerDescProgressBar",
  "Glass background": "lv_layerDescGlassPanel",
  "Ô rank/level": "lv_layerDescStatCapsule",
  "Text #rank": "lv_layerDescRankText",
  "Text level": "lv_layerDescLevelText",
  "Text XP/progress": "lv_layerDescXpText",
};
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <Input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); add(); } }} placeholder={t("lv_discordId")} />
  ...
  return <div className="space-y-4 sm:space-y-6">
    <Tabs value={section || "rank-card"} className="space-y-4">
                ...
                <div className="space-y-5">
                  ...
                  <div className="rounded-2xl border bg-muted/20 p-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <div><Label className="text-xs">{t("lv_xpLabel")}</Label><Input value={rankCard.xp_label ?? ""} onChange={e => setRankCard({ ...rankCard, xp_label: e.target.value })} placeholder={t("lv_xpLabel")} /></div>
                    </div>
                  </div>
        ...
        <div className="hidden">
            {rankCard && <div className="grid gap-3 rounded-2xl border bg-background/70 p-3 sm:rounded-3xl sm:p-4 xl:grid-cols-[1fr_1fr_auto]">
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div><Label>{t("lv_uploadBg")}</Label><Select value={rankCard.background ?? ""} onValueChange={v => setRankCard({...rankCard, background: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="aurora">{t("lv_aurora")}</SelectItem><SelectItem value="midnight">{t("lv_midnight")}</SelectItem><SelectItem value="sunset">{t("lv_sunset")}</SelectItem><SelectItem value="mono">{t("lv_mono")}</SelectItem></SelectContent></Select></div>
                <div><Label>{t("lv_canvasEditor")}</Label><Select value={rankCard.panel_style ?? ""} onValueChange={v => setRankCard({...rankCard, panel_style: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="glass">{t("lv_glass")}</SelectItem><SelectItem value="solid">{t("lv_solid")}</SelectItem></SelectContent></Select></div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            {rankCard && layout && <div className="grid gap-4 xl:min-h-[560px] xl:grid-cols-[240px_minmax(0,1fr)_300px]">
              <div className="rounded-2xl border bg-background/80 p-3 sm:rounded-3xl">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button size="sm" variant="secondary" onClick={() => addLayer("text")}><Type className="mr-1 h-3 w-3" />{t("lv_layerText")}</Button>
                  <Button size="sm" variant="secondary" onClick={() => addLayer("rect")}><Square className="mr-1 h-3 w-3" />{t("lv_layerShape")}</Button>
                  <Button size="sm" variant="secondary" onClick={() => addLayer("avatar")}><CircleUserRound className="mr-1 h-3 w-3" />{t("lv_layerAvatar")}</Button>
                  <Button size="sm" variant="secondary" onClick={() => addLayer("progress")}><Zap className="mr-1 h-3 w-3" />{t("lv_layerProgress")}</Button>
                </div>
                  ...
                  <div className="max-h-44 space-y-1 overflow-y-auto pr-1">
                    {LAYER_PRESETS.map((preset) => (
                      <button key={`${preset.type}-${preset.name}`} type="button" onClick={() => addPresetLayer(preset)} className="group flex w-full items-center justify-between gap-2 rounded-xl px-2 py-2 text-left transition hover:bg-background">
                        <span className="min-w-0"><span className="block truncate text-xs font-medium">{t(LAYER_NAME_KEYS[preset.name] || preset.name)}</span><span className="block truncate text-[11px] text-muted-foreground">{t(LAYER_DESC_KEYS[preset.desc] || preset.desc)}</span></span>
                ...
                <div className="mt-4 max-h-56 space-y-1 overflow-y-auto pr-1 sm:max-h-72 xl:max-h-[280px]">
                  {[...layout.layers].sort((a, b) => b.z - a.z).map((layer) => (
                    <button key={layer.id} onClick={() => setSelectedLayerId(layer.id)} className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm transition ${selectedLayer?.id === layer.id ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted"}`}>
                      <span className="truncate"><span className="mr-2 opacity-60">{layer.type}</span>{t(LAYER_NAME_KEYS[layer.name] || layer.name)}</span>
              ...
              <div className="min-w-0 rounded-2xl border bg-background/80 p-3 sm:rounded-3xl sm:p-4">
                <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                    ...
                    <Select value={layout.background.style} onValueChange={v => updateLayout({background: {...layout.background, style: v}})}><SelectTrigger className="h-9 w-full sm:w-28"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="solid">{t("lv_solid")}</SelectItem><SelectItem value="gradient">{t("lv_gradient")}</SelectItem></SelectContent></Select>
              ...
              <div className="rounded-2xl border bg-background/80 p-3 sm:rounded-3xl sm:p-4">
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div><p className="text-base font-semibold">{t("lv_canvasEditor")}</p><p className="text-sm text-muted-foreground">{selectedLayer ? t(LAYER_NAME_KEYS[selectedLayer.name] || selectedLayer.name) : t("lv_canvasDesc")}</p></div>
                ...
                <div className="space-y-3">
                  {selectedLayer ? <>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2"><div><Label>{t("lv_propX")}</Label><Input type="number" value={selectedLayer.x ?? ""} onChange={e => updateSelectedLayer({x: Number(e.target.value)})} /></div><div><Label>{t("lv_propY")}</Label><Input type="number" value={selectedLayer.y ?? ""} onChange={e => updateSelectedLayer({y: Number(e.target.value)})} /></div><div><Label>{t("lv_propW")}</Label><Input type="number" value={selectedLayer.w ?? ""} onChange={e => updateSelectedLayer({w: Number(e.target.value)})} /></div><div><Label>{t("lv_propH")}</Label><Input type="number" value={selectedLayer.h ?? ""} onChange={e => updateSelectedLayer({h: Number(e.target.value)})} /></div><div><Label>{t("lv_propZ")}</Label><Input type="number" value={selectedLayer.z ?? ""} onChange={e => updateSelectedLayer({z: Number(e.target.value)})} /></div><div><Label>{t("lv_propOpacity")}</Label><Input type="number" min={0} max={100} value={selectedLayer.opacity ?? 100} onChange={e => updateSelectedLayer({opacity: Number(e.target.value)})} /></div></div>
                    <div className="grid gap-2 sm:grid-cols-2"><div><Label>{t("lv_propRadius")}</Label><Input type="number" value={selectedLayer.radius ?? 0} onChange={e => updateSelectedLayer({radius: Number(e.target.value)})} /></div><div><Label>{t("lv_propColor")}</Label><Input type="color" value={selectedLayer.color || selectedLayer.fill || "#ffffff"} onChange={e => updateSelectedLayer(selectedLayer.type === "progress" ? {fill: e.target.value} : {color: e.target.value})} /></div></div>
                    {selectedLayer.type === "text" && <div className="grid gap-2 sm:grid-cols-2"><div><Label>{t("lv_propToken")}</Label><Select value={selectedLayer.token || "display_name"} onValueChange={v => updateSelectedLayer({token: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["display_name","username","server","level","rank","xp","progress","percent"].map(tk => <SelectItem key={tk} value={tk}>{tk}</SelectItem>)}</SelectContent></Select></div><div><Label>{t("lv_propFontSize")}</Label><Input type="number" value={selectedLayer.font_size || 24} onChange={e => updateSelectedLayer({font_size: Number(e.target.value)})} /></div><div className="flex items-center justify-between rounded-xl border px-3 py-2 sm:col-span-2"><Label>{t("lv_propBold")}</Label><Switch checked={Boolean(selectedLayer.bold)} onCheckedChange={v => updateSelectedLayer({bold: v})} /></div></div>}
      ...
      <TabsContent value="config"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Settings className="w-4 h-4" /> {t("lv_xpConfig")}</CardTitle><CardDescription>{t("lv_adjustXpDesc")}</CardDescription></CardHeader><CardContent className="space-y-5">
        ...
        <div className="grid md:grid-cols-2 gap-4"><div><Label>{t("lv_notifyMode")}</Label><Select value={form.level_up_mode ?? ""} onValueChange={v => setForm({...form, level_up_mode: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="current">{t("channel")}</SelectItem><SelectItem value="fixed">{t("lv_levelUpChannel")}</SelectItem><SelectItem value="dm">{t("lv_dm")}</SelectItem><SelectItem value="off">{t("disabled")}</SelectItem></SelectContent></Select></div><div><Label>{t("lv_levelUpChannel")}</Label><ChannelSelect value={form.level_up_channel_id || ""} onChange={v => setForm({...form, level_up_channel_id: v})} filter="text" /></div></div>
        <div className="rounded-2xl border bg-muted/20 p-4">

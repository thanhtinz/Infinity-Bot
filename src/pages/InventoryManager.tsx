...
export function InventoryManager() {
  ...
  // Queries
  const { data: stats = [], isLoading: statsLoading } = useQuery<InventoryStat[]>({
    queryKey: ["inventory-stats"],
    queryFn: () => apiFetch("/api/inventory/stats").then((r) => r.json()),
  });
  ...
  const { data: items = [], isLoading: itemsLoading } = useQuery<InventoryItem[]>({
    queryKey: ["inventory", filterProduct, filterPackage, filterStatus],
    queryFn: () => apiFetch(`/api/inventory?${queryParams.toString()}`).then((r) => r.json()),
  });
  ...
  async function saveThreshold() {
    ...
    try {
      const r = await apiFetch("/api/config", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ inventory_low_stock_threshold: threshold }),
      });
      if (!r.ok) throw new Error();
      toast({ title: "Threshold saved." });
    } catch {
      toast({ variant: "destructive", title: "Error saving threshold." });
    } finally {
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="flex items-center gap-3">
            <Label className="text-sm font-medium whitespace-nowrap">Low stock warning threshold:</Label>
            <Input
              type="number"
              min={0}
              value={threshold}
              ...
              className="w-24 h-8"
            />
            <Button size="sm" onClick={saveThreshold} disabled={thresholdSaving}>
              {thresholdSaving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Save"}
            </Button>
          </div>
        </CardContent>
      </Card>

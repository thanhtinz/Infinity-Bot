...
export const VARIABLE_GROUPS: { label: string; icon: typeof Variable; vars: { key: string; desc: string }[] }[] = [
  ...
  },
  {
    ...
    vars: [
      ...
      { key: "{unix}", desc: "Unix timestamp (seconds)" },
      { key: "{day}", desc: "Day of week (Monday…)" },
      { key: "{month}", desc: "Month name (January…)" },
  ...
  },
  {
    ...
    icon: Variable,
    vars: [
      { key: "{math:1+2}", desc: "Evaluate math expression" },
      { key: "{random:1-100}", desc: "Random number in range" },
      { key: "{counter}", desc: "Auto-increment counter" },
    ],
  },
  {
    label: "Conditions",
    icon: Variable,
    vars: [
      { key: "{if:{user.id}==123:yes:no}", desc: "If/else condition" },
      { key: "{hasrole:RoleName:yes:no}", desc: "Check if user has role" },
      { key: "{isboosting:yes:no}", desc: "Check if user is boosting" },
      { key: "{isnew:7:yes:no}", desc: "Account newer than N days" },
    ],
  },
  {
    label: "Other",
    ...
    vars: [
      ...
      { key: "$3", desc: "Argument 3" },
      { key: "$1+", desc: "All arguments from 1" },
      { key: "$2+", desc: "All arguments from 2" },
      { key: "{args}", desc: "All arguments joined" },
      ...
      { key: "{embed}", desc: "Send as embed (plain text → embed)" },
      { key: "{ephemeral}", desc: "Visible only to command user" },
      { key: "{typing}", desc: "Show typing indicator" },
      { key: "{react:👍}", desc: "React to message with emoji" },
    ],
  },
];

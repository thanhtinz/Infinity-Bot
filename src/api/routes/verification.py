...
def get_config(guild_id: str = Depends(get_guild_id), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "content_opacity": getattr(cfg, "content_opacity", 100) if getattr(cfg, "content_opacity", None) is not None else 100,
...
async def update_config(body: dict, guild_id: str = Depends(get_guild_id), db: Session = Depends(get_db)):
    ...
    allowed = [
        ...
        "card_border_color", "card_bg_color", "card_opacity", "content_opacity",
        "typewriter_effect", "typewriter_desc_effect", "glow_effect", "tilt_effect",
